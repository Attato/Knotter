...
function EditMode({ node, isSelected }: NodeProps) {
    ...
    return (
        <div
            ref={nodeRef}
            className={`
                relative flex flex-col gap-1 max-w-96 min-w-16 w-full border-1 rounded-lg bg-background text-sm p-1 hover:cursor-move
...
function ViewMode({ node, isSelected }: NodeProps) {
    ...
    return (
        <div className="relative w-full h-full flex items-center justify-center">
            <Icon
                className={`w-full h-full fill-background hover:cursor-move 
