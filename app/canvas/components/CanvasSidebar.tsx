...
const SIDEBAR_COMPACT_THRESHOLD = 60;
const SIDEBAR_BASE_WIDTH = 480;

export default function CanvasSidebar() {
    const { nodes, setNodes } = useCanvasStore();
    const [filterText, setFilterText] = useState('');
    const { width, isCompact, isResizing, startResize } = useSidebarResize(SIDEBAR_BASE_WIDTH, SIDEBAR_COMPACT_THRESHOLD);
    ...
    return (
        <aside style={{ width }} className="h-screen border-l border-[#1a1a1a] bg-[#0f0f0f] select-none flex-shrink-0">
            <div className="flex flex-col h-full overflow-hidden relative">
                ...
                {isCompact && (
                    <div className="flex flex-col justify-between h-full items-center gap-2 m-1">
                        <button
                            onClick={addNode}
                            className="w-fit bg-[#151515] hover:bg-[#1a1a1a] p-2 rounded-md transition cursor-pointer"
                        >
